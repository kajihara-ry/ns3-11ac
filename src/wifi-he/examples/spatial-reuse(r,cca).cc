  ...
  if (useIdealWifiManager == false) //spatial-reuseではuseIdealWifiManager == falseでシミュレート
    {
      ...
      StringValue ctrlRate2; //Multi-BSS scenario (BSS2)
...
//PHY rateを定義する関数は外部に存在（wifi-mode.cc?）するので2種類設定するには外部を書き換える必要あり？ Control Frame Rateを変える必要あり？
      wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager","DataMode", StringValue (dataRate1),
...
////////////////////////////////////////////////////////////////////CCA-ED、PHYのパワーを設定（ここから）/////////////////////////////////////////////////////////////////////////////////////////
  ...
  spectrumPhy.Set ("CcaEdThreshold", DoubleValue (ccaTrSta1)); //Multi-BSS scenario (BSS1)
  ...
  if (nBss >= 3)
    {
      ...
      spectrumPhy.Set ("MaxSupportedTxSpatialStreams", UintegerValue (maxSupportedTxSpatialStreams));
      spectrumPhy.Set ("MaxSupportedRxSpatialStreams", UintegerValue (maxSupportedRxSpatialStreams));
      spectrumPhy.Set ("CcaEdThreshold", DoubleValue (ccaTrSta)); //使わない(ccaTrSta)
      spectrumPhy.Set ("RxSensitivity", DoubleValue (rxSensitivity));
...
Config::Disconnect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BE_Txop/CwTrace", MakeCallback (&CwTrace));
  ...
  SaveSpatialReuseStats (outputFilePrefix + "-SR-stats-" + testname + ".dat", d,  r1, r2, freq, csr, scenario, aggregateUplinkMbps, aggregateDownlinkMbps); //Multi-BSS scenario(r1とr2を保存するように変更)
