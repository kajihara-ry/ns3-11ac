...
uint64_t
WifiMode::GetPhyRate (uint16_t channelWidth, uint16_t guardInterval, uint8_t nss) const //PHY rateを返す関数?(nss:空間ストリーム数)
{
...
WifiMode::GetCodeRate (void) const
{
  ...
  else if (item->modClass == WIFI_MOD_CLASS_VHT) //VHTではMCSはCoding Rateとして使用,PHY rateは計算する
    {
